...
- DRAM으로 구성된 Main Memory에 접근하려면 수십 사이클에서 100 사이클까지 걸리는데 자주 사용하는것을 더 가까이 두면 굳이 다시 Main Memory에 접근하지 않아도 되기 때문에 CPU의 성능 향상을 위해 캐시메모리를 두고, 자주 사용하는 Instruction을 넣는다.
...
- 더 정확하게는 물리적인 메모리에 바로 올라가는것이 아니라 중간에 한 단계를 더 거치게 되는데, 그것은 해당 프로그램만의 독자적인 메모리 주소 공간이 형성되는 가상 메모리(Virtual memory)단계 이다.
    ...
    - 위의 이미지에서는 code영역이 끝나면 data영역이 시작되는것처럼 보이지만, 실제로는 순차적으로 연결되어있지 않고 code영역은 물리적 메모리에, 나머지 당장 사용되지 않는 정보들은 디스크의 Swap Area에 있다고 생각하면 된다.
...
- 메모리 낭비 때문에 사용자 프로그램을 실행 시켰을때 생긴 메모리 주소 공간 전체가 물리적 메모리에 올라가는것은 아니다!
    - 보통 가상 메모리의 code영역만 물리적 메모리에 올려 놓는다.
    - 당장 필요하지 않은 정보들은 디스크의 Swap Area에 내려 놓는다.
