...
- **CPU는 항상 Main Memory의 Instruction만을 읽어들이는 작업을 수행한다고 했다.**
    ...
    - Instruction을 읽어들이는 작업은 아래와 같다.
        - Instruction 하나가 실행되고 나면 program counter 내 주소값이 증가하고 다음 클럭 사이클에서는 증가한 주소값에 있는 Instruction을 실행한다.
- 그럼 인터럽트 라인은 무엇인가?
    - **CPU는 하나의 Instruction을 수행하고 항상 확인하는것이 있는데, 바로 Interrupt Line이다.**
- **Interrupt Line은 말 그대로 CPU를 Interrupt하기 위한 Line이고 이 라인에 무엇인가 들어오면 CPU는 하던일을 멈추고 OS에게 자신의 소유권을 넘긴다.**
...
- 프로그램이 CPU에서 Instruction을 수행하다가 프로그램 A가 유저로 부터 입력값을 원하는 Scanf와 같은 Instruction을 읽으면 키보드 입력을 해야한다.
    - 이때 발생하는 일은 아래와 같다. (Mode Bit, Trap(System Call),을 제외한 간단한 설명)
...
## 🍎 Timer
- timer의 역할은 **특정 프로그램이 CPU를 독점하는 것으로 부터 보호**
...
- memory controller의 정확한 역할은 DMA Controller를 설명할 때 나온다.
...
- OS는 CPU를 통해 I/O에게 입력을 받아오라고 시킨다.
- CPU는 자신의 Mode Bit이 0인지 확인하고 Device Controller 내 control register에게 업무를 전달한다.
    - I/O는 실제 device(예. 키보드)와 local buffer 사이에서 일어난다!
...
- DMA Controller는 왜 필요할까?
    ...
    - CPU가 I/O interrupt가 올때마다 local buffer에서 데이터를 가지고 입력을 요구했던 사용자 프로그램의 메모리에 카피까지 해주는 역할을 하는것은 오버헤드가 크다.
...
- 사용자 프로그램은 어떻게 I/O를 하는가?
    - 시스템 콜(system call)
        ...
        - Mode Bit이 1일때는 특권명령을 수행할 수 없으므로 사용자 프로그램은 운영체제에게 I/O 요청한다.
