...
### 📖 실제 프로세스 생성 과정
- **운영체제가 부모 프로세스의 주소공간(코드,데이터,스택) + 프로그램 카운터를 하나의 프로세스에 그대로 복사해 생성한다 이것을 자식 프로세스라고 한다.** -> fork()
- **자식 프로세스는 그 주소공간에 새로운 프로그램을 올린다** (덮어 씌운다). -> exec()
- 유닉스의 예
...
## 🍎 exit() 시스템 콜
- 보통의 상황에서는 프로세스의 마지막에 자발적으로 종료되게끔 되어있다.
- 자발적 종료
